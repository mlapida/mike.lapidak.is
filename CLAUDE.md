...
- Weight-token cleanup: the kit serves Parisine only at 400/700, so the 300/500/600 weights used in CSS render as 400/700 (browsers snap to nearest). Either collapse the tokens in `global.css` + the design.md weight table to 400/700, or deliberately add Parisine Clair/Gris/Sombre (separate families, not weights)
