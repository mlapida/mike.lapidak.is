...
| `public/social-card.jpg` | Default OG/Twitter card image (generated: ML monogram plate + domain on paper, 1200x630) |
| `public/post-images/<slug>/*` | Body images referenced by post markdown (stable URLs for the `.md` companion routes) |
| `src/assets/post-images/<slug>/feature.<ext>` | Feature/hero images · run through Astro's image pipeline |
...
**Body images** live at `public/post-images/<slug>/<filename>` (stable URLs so the `/posts/<slug>.md` companion routes keep working; a rehype plugin at `src/lib/rehype-post-images.mjs` adds width/height + lazy loading at build). **Feature/hero images** live at `src/assets/post-images/<slug>/feature.<ext>` and run through Astro's image pipeline; `src/lib/feature-images.ts` maps the frontmatter `feature_image` path (still `/post-images/<slug>/feature.<ext>`, used as an identifier) to the imported asset. The detail page renders the feature image at the top of the article (16:9, `object-fit: cover`); OG/Twitter, Article JSON-LD, and the RSS enclosure share one pipeline-generated 1200px JPEG.
...
The original feature/hero images were imported via a one-shot script (deleted after use). Future post additions should commit feature images to `src/assets/post-images/<slug>/feature.<ext>` and reference them via the `feature_image` frontmatter field (which keeps the `/post-images/<slug>/feature.<ext>` path format as an identifier). Oversized body screenshots should be WebP; the 2022-23 CleanShot PNGs were batch-converted (38 files, 57.5 MB to 3.6 MB).
...
`public/`: `favicon.svg` (crisp, glyph outlines), `favicon-16x16.png`, `favicon-32x32.png`, `apple-touch-icon.png`, `favicon.ico`. All linked in `Base.astro`. All carry the ML monogram (italic serif on ink plate, matching the nav mark); regenerate with sharp + fontTools glyph extraction if the mark changes.
...
| File | What it covers |
|------|---------------|
| `routes.test.ts` | Top-level pages + `404.html`, all 17 photo detail routes, both collection routes; `/posts/` index + pagination (`/posts/2/`, `/posts/3/`); 24 published post detail dirs; every detail has a sibling `.md` companion; `/rss.xml` exists; post-detail hero renders from `/_astro/feature.*` with srcset + fetchpriority; homepage Writing feed thumbnails are pipeline-generated `/_astro/feature.*.webp` |
| `sitemap.test.ts` | `sitemap-index.xml` and `sitemap-0.xml` structure; total `<loc>` count is 49; contains `/posts/`, `/posts/2/`, `/posts/3/`, sample post slugs, all photos and collections; does NOT contain `/rss.xml`, `/llms.txt`, or `.md` companions |
...
- Cloudflare dashboard: disable Rocket Loader (the site ships ~2 KB of JS; the inline theme script already carries `data-cfasync="false"` as defense), then drop `https://ajax.cloudflare.com` from `script-src` in `public/_headers`
- Adobe Fonts dashboard: prune kit `mjy4jau` to the faces in use (parisine-std 400/700 + adobe-caslon-pro italic; the kit currently declares 34 faces across 13 families) and set kit font-display to `swap`. Note: the kit serves Parisine only at 400/700, so the 300/500/600 weights in `global.css` are browser-synthesized today — either embrace 400/700 or add the real faces deliberately
- Polaroid talk thumbnails on /work (pre-approved in design.md, needs session/venue imagery)
